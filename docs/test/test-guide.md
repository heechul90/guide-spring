...
## 테스트 대역(Test Double)
외부 의존 구성요소를 사용할 수 없거나 직접 사용하고 싶지 않을 때, 테스트 대상 코드와 대신해서 상호작용하는 객체를 말합니다.
...
### 1. 통합 테스트
통합테스트는 테스트하고자 하는 코드가 각 모듈들의 상호 작용이 제대로 이루어지는지 검증하는 테스트입니다.

#### 장점
- 스프링부트 컨테이너를 동작시켜 테스트하기 때문에 운영환경과 가장 유사한 테스트가 가능합니다.
- 전체적인 Flow 를 쉽게 테스트 할 수 있습니다.

#### 단점
- 애플리케이션의 설정 및 모든 Bean 을 로드하기 때문에 시간이 오래걸리고 무겁습니다.
- 테스트 단위가 커 디버깅이 어렵습니다.
- 외부 API 콜같은 Rollback 처리가 안되는 테스트는 진행하기 어렵습니다.
...
- `@AutoConfigureMockMvc` 어노테이션을 통해 `@Service` 나 `@Repository` 가 붙은 객체들도 메모리에 올립니다.
...
- Given-When-Then 패턴으로 Test Code 를 작성합니다.
  - Given : 테스트에 사용할 User 를 EntityManager 를 통해 영속성 컨텍스트를 준비합니다.
  - When : MockMvcRequestBuilders 클래스를 사용하여 실제로 테스트를 실행합니다.
  - Then : MockMvcResultMatchers 클래스를 사용하여 테스트를 검증합니다.

### 2. 단위 테스트
단위테스트는 소스 코드의 독립된 특정 모듈을 개별적으로 검증하는 테스트입니다.

#### 장점
- 테스트 시간 단축으로 문제점 발견 가능성이 높아지고 안정성이 향상됩니다.
- 테스트 간 결합도가 낮으므로 간단해집니다.

#### 단점
- 의존성 있는 객체를 Mocking 하기 때문에 완결한 테스트가 아닙니다.
...
```
- 레포지토리테스트의 Base 클래스
- `@DataJpaTest` 어노테이션을 통해 jpa repository 테스트를 선언합니다. 
- `@AutoConfigureTestDatabase` 어노테이션을 통해 데이터베이스 설정을 변경합니다.
